#----------------------------------------------------------------------------
# same as refund::gibbs_mult_fpca but modified to store MCMC draws of the coefficients
#----------------------------------------------------------------------------
...
#' @param Kp number of FPCA basis founctions to be estimated
...
#' @export
#' 
gibbs_mult_fpca2 = function(formula, Kt=5, Kp = 2, data=NULL, verbose = TRUE, N.iter = 5000, N.burn = 1000, 
  ...
  ret = list(beta.pm, beta.UB, beta.LB, fixef.pm + ranef.pm, ranef.pm, mt_fixed, data, psi.pm, BW, Theta)
  names(ret) = c("beta.hat", "beta.UB", "beta.LB", "Yhat", "ranef", "terms", "data", "psi.pm", "BW", "Theta")
  class(ret) = "fosr"
  ret
