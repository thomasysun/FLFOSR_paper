...
fgee <- function(formula, Y, Cov, s, subjID, Tij, numLongiPoints = 41, 
  ...
  plot.data <- {
    pdf(NULL)
    plot.init <- plot(fit_init,n=length(s), seWithMean=TRUE, pages=10)
    invisible(dev.off())
  }
  ...
  if(corstr == "unspecified"){
    ...
    for(k in 1:ncol(phi)){
      ...
      for(j in 1:ncol(psi.MF[[k]])){
        ...
        if(lambda.psi[[k]][j] >= max(threshold, 0.05*lambda.sum)){ 
          index <- index + 1
          assign(paste("efunc",index,sep=""), efuncs[[k]][,j], envir = parent.frame(2)) # make it global variable
        }
      }
    }
    ...
    for(k in 1:ncol(phi)){
      ...
      index2 <- index2 + 1
      assign(paste("efunc2",index2,sep=""), efuncs2[[k]], envir = parent.frame(2)) ##make it global variable
    }
  ...
  if(corstr == "exchangeable"){
    ...
    for(k in 1:ncol(phi)){
      ...
      index <- index + 1
      assign(paste("efunc",index,sep=""), efuncs[[k]], envir = parent.frame(2)) ##make it global variable
    } 
  ...
  plot.data <- {
    pdf(NULL)
    plot <- plot(fit,n=length(s),seWithMean=TRUE, pages=10)
    invisible(dev.off())
  }
